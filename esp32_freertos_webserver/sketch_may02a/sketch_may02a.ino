...
void setup() {
 ...
 digitalWrite(ledPin, LOW);
  ...
  xTaskCreatePinnedToCore(Task_readKeypadTask, "KeypadTask", 2048, NULL, 5, NULL,1);
      xTaskCreatePinnedToCore(Task_readbuttonled, "buttonledTask", 2048, NULL, 5, NULL,1);
   xTaskCreatePinnedToCore(Task_readbuttonfan, "buttonfanTask", 2048, NULL, 5, NULL,1);
     xTaskCreatePinnedToCore(Task_readbuttonpump, "buttonpumpTask", 2048, NULL, 5, NULL,1);
...
void trangthailed()
{
  ...
  server.send(200, "text/plane", thled);
}

void trangthaifan()
{
  server.send(200, "text/plane", thfan);
}

void handletrangthaipump()
{

  server.send(200, "text/plane", thpump);
}
*/
...
// ----------------------------------- hamf gui trang thai cua nut nhan tu server len client
...
void Task_readbuttonled(void *pvParameters) { // Task to read keypad input
 ...
 hientailed = digitalRead(nutnhanled);

 //delay (500);
 
 if(hientailed != macdinhled)
 {
  if(hientailed == 0)
    {
      demled+=1;
    }
  macdinhled = hientailed;
 }
...
void Task_readbuttonfan(void *pvParameters) { // Task to read keypad input
 ...
 hientaifan = digitalRead(nutnhanfan);
 

 //delay (500);
 
 if(hientaifan != macdinhfan)
 {
  if(hientaifan == 0)
    {
      demfan+=1;
    }
  macdinhfan = hientaifan;
 }
...
void Task_readbuttonpump(void *pvParameters) { // Task to read keypad input
 ...
 hientaipump = digitalRead(nutnhanpump);
 

 //delay (500);
 
 if(hientaipump != macdinhpump)
 {
  if(hientaipump == 0)
    {
      dempump+=1;
    }
  macdinhpump = hientaipump;
 }
